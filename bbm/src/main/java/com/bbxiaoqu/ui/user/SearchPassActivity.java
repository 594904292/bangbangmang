...
public class SearchPassActivity extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		register_btn.setOnClickListener(new OnClickListener() {
			public void onClick(View v) {
				...
				if(pass.equals(""))
				{
					Toast.makeText(SearchPassActivity.this, "请填写用户密码", Toast.LENGTH_LONG);
					return;
				}
				if(tlephone.equals(""))
				{
	...
	Handler savehandler = new Handler() {
		...
		public void handleMessage(Message msg) {
			...
			if (result == 1) {
				Toast.makeText(SearchPassActivity.this, "保存成功",Toast.LENGTH_SHORT).show();
				
				UserService uService = new UserService(SearchPassActivity.this);
				User user = new User();
				...
				Toast.makeText(SearchPassActivity.this, "注册成功", Toast.LENGTH_LONG).show();
				
				Intent intent = new Intent(SearchPassActivity.this,LoginActivity.class);
				startActivity(intent);
			}else if (result == 2) {
				Toast.makeText(SearchPassActivity.this, "用户ID已注册",Toast.LENGTH_SHORT).show();
			}else if (result == 3) {
				Toast.makeText(SearchPassActivity.this, "手机号已注册",Toast.LENGTH_SHORT).show();
			} else if (result == 4) {
				Toast.makeText(SearchPassActivity.this, "验证码不正确",Toast.LENGTH_SHORT).show();
			} else {
				Toast.makeText(SearchPassActivity.this, "保存失败",Toast.LENGTH_SHORT).show();
			}
		}
	};
